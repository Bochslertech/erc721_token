...
shared (install) actor class ERC721(init_minter: Principal) = this {
  ...
  public shared (msg) func initproperties(start : TokenIndex, end : TokenIndex) : async () {
    assert (msg.caller == Principal.fromText("6vatw-256w7-b4cy5-562mf-vtv5l-shaj6-x6yr4-mdpsc-ccjw4-yuini-mae"));
  ...
  public shared (msg) func clearProperties() {
    assert (msg.caller == Principal.fromText("6vatw-256w7-b4cy5-562mf-vtv5l-shaj6-x6yr4-mdpsc-ccjw4-yuini-mae"));
    _properties := HashMap.HashMap<Property, [TokenIndex]>(0, _equal, _hash);
  };
  ...
  public shared (msg) func updateNFTName(originnalName : Text, updateName : Text, start : TokenIndex, end : TokenIndex) : async () {
    assert (msg.caller == Principal.fromText("6vatw-256w7-b4cy5-562mf-vtv5l-shaj6-x6yr4-mdpsc-ccjw4-yuini-mae"));
    label c for (i in Iter.range(Nat32.toNat(start), Nat32.toNat(end))) {
      let tokenIndex = Nat32.fromNat(i);
      switch (_tokenMetadata.get(tokenIndex)) {
        case (? #nonfungible(metadata)) {
          let json = switch (metadata.metadata) {
            case (?blob) {
              switch (Text.decodeUtf8(blob)) {
                case (?text) {
                  text;
          ...
          switch (p.parse(json)) {
            case (? #Object(v)) {
              var newProperties : [Json.JSON] = [];
              switch (v.get("attributes")) {
                case (? #Array(pv)) {
                  for (info in pv.vals()) {
                    switch (info) {
                      case (#Object(detail)) {
                        switch (detail.get("value")) {
                          case (? #String(value)) {
                            if ((value) == originnalName) {
                              detail.put("value", #String(updateName));
                            };
                            // put array
                            newProperties := Array.append<Json.JSON>(newProperties, [#Object(detail)]);
                          };
                          case _ {};
                        };
                      };
              ...
              if (newProperties.size() > 0) {
                v.put("attributes", #Array(newProperties));
              };
              Debug.print(debug_show ("json", Json.show(#Object(v))));
              let metadata : ?Blob = ?Text.encodeUtf8(Json.show(#Object(v)));
              let md : Metadata = #nonfungible({
                metadata = metadata;
              });
              _tokenMetadata.put(tokenIndex, md);
            };
            case _ {};
          };
        };
